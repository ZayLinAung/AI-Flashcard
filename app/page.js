...
import { SignIn, SignedOut, SignInButton, SignOutButton, SignUpButton, SignedIn } from "@clerk/nextjs";
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      <header className="flex justify-between items-center p-6 bg-white dark:bg-gray-800 ">
        <Link href="#" className="flex items-center text-4xl font-bold" prefetch={false}>
          <span>CramIt</span>
        </Link>
        <nav className="space-x-4">

          {/* Display when users are logged out */}
          <Link href="#" className="text-lg text-gray-700 dark:text-white" prefetch={false}>
            <SignedOut>
              <SignInButton />
            </SignedOut>
          </Link>
